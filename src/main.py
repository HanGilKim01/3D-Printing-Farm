from config import *  # 시뮬레이션 설정 및 구성 정보
import Env as env  # 환경 생성 및 프로세스 정의 (수정된 create_env와 simpy_event_processes 포함)
from log import *  # 로그 및 이벤트 기록
import pandas as pd  # 데이터 분석 및 저장
import visualization

# Step 1: 환경 및 객체 초기화
# 수정된 create_env는 아래 순서대로 반환합니다.
# (simpy_env, packaging, dry_machine, washing_machine, post_processor, customer, printers, daily_events)
(simpy_env, packaging, dry_machine, washing_machine, post_processor, customer, printers, daily_events) = env.create_env(DAILY_EVENTS)

# Step 2: SimPy 이벤트 프로세스 설정
# (PostProcessing는 Drying에서 호출되고, Packaging는 PostProcessing 내부로 전달되므로 별도 실행은 필요하지 않습니다.)
env.simpy_event_processes(simpy_env, packaging, post_processor, customer, printers, daily_events)
